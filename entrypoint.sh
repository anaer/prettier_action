...
if _git_changed; then
  if $INPUT_DRY; then
    ...
    if $INPUT_SAME_COMMIT; then
      ...
      git commit -m "$INPUT_COMMIT_MESSAGE" --author="$INPUT_COMMIT_USER_NAME <$INPUT_COMMIT_USER_EMAIL>" ${INPUT_COMMIT_OPTIONS:+"$INPUT_COMMIT_OPTIONS"} || echo "No files added to commit"
      git push origin
    fi
